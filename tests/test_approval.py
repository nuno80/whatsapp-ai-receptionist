...
async def test_create_request_fans_out(mock_redis, mock_config, mock_whatsapp):
    ...
    assert mock_whatsapp.send_text.call_count == 2
    # Check that message contains the ID
    first_call_args = mock_whatsapp.send_text.call_args_list[0][1]
...
async def test_handle_approval_multiple_pending_requires_id(mock_redis, mock_config, mock_whatsapp):
    ...
    mock_whatsapp.send_text.assert_called_with(to="393000000001", text=pytest.approx("Ci sono più richieste in attesa. Rispondi con OK <id> o NO <id>:\n- req1\n- req2"))
...
async def test_handle_approval_claim_wins_ok(mock_redis, mock_config, mock_whatsapp, mocker):
    ...
    # Guest notified
    guest_call = [call for call in mock_whatsapp.send_text.call_args_list if call.kwargs['to'] == '+1234'][0]
    assert "Confermata" in guest_call.kwargs['text']
    
    # Other approver notified
    other_call = [call for call in mock_whatsapp.send_text.call_args_list if call.kwargs['to'] == '393000000002'][0]
...
async def test_handle_approval_claim_loses(mock_redis, mock_config, mock_whatsapp):
    ...
    mock_whatsapp.send_text.assert_any_call(to="393000000001", text="Richiesta r123 già gestita da Marco.")
